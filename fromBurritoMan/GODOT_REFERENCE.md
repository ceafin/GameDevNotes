# Godot Dev Reference (for Claude)

Working reference for breaking down games, structuring code, and staying out of your own head. Target: **Godot 4.6.x, GDScript, static typing on.** One principle repeated: *local things stay local; only the genuinely global gets lifted out, minimally, when the pain is real.*

> Correctness pass applied. Notable fixes vs the old card: `Tween` is for fixed A->B moves, not following a moving target; `change_scene_to_file()` frees only the current scene branch; siblings talk through their parent by default, not through global state.

---

## Your Two Traps (re-read first)

You oscillate between two opposite mistakes. The middle path for both is **the tree**.

- **Trap A, over-building up front.** Designing the "perfect reusable foundation" on a blank page. You can't; you have no evidence yet for what you'll need. The foundation *crystallizes from repetition*. Write the dumb specific version 3x, then extract the pattern.
...
- **Rule for both:** build or lift structure *when you hit a concrete wall*, never preemptively. If you're doing it "for flexibility later," stop.

---

## Mindset

- **Godot IS your pre-built ecosystem.** Node tree = entity list. `_process` / `_physics_process` = tick loop. Signals = event system. Resources = data files. You add *content*, not scaffolding.
- **Rebuilding a built-in is the blank-page problem in disguise.** Both are "I built the foundation that already existed."
...
Before coding a mechanic, search the docs for the **noun** of what you want.
...
- **`change_scene_to_file()` frees the current scene branch** (the old scene and all its children). The **root and autoloads survive.** Anything stored "in the level" or "on the player" node dies; autoload data does not.
...
- **Subtlety is the actual skill:** Mega Man *health* is scene-local, *lives* are session-global. Same kind of number, opposite bucket. Draw the line per variable.
...
Don't file these in the same drawer. Mixing them is what made it feel like "autoloaders everywhere."
...
- **Call down** = parent calls methods on children it holds references to. `enemy.take_damage(10)`, `hud.set_health(80)`. Not a compromise, it is correct. Stop feeling guilty.
...
3. **Neither, a genuinely game-wide moment with no owner?** -> **Tier 3: SignalBus, sparingly.** `player_died`, `game_paused`, `boss_defeated`. About 8 signals in a finished Zelda clone, not 80.

- **SignalBus-for-everything was not wrong because the game got big.** It was fine for Pong *because* Pong was small enough to hide that the bus eats the tree's directionality.
